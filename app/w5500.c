...
unsigned char Phy_Addr[PHY_LEN];	//物理地址(MAC) 
unsigned char IP_Addr[8];	//本机IP地址 
...
volatile uint8_t W5500_Interrupt = 0;

void Delay(unsigned int d)
{
	//vTaskDelay(d);
	uint32_t tmp = d*10000;
	while(tmp--);
}
/*******************************************************************************
...
void Load_Net_Parameters(void)
{
	...
	flash_read_pages(CFG_NET_PAGE,(uint8_t*)IP_Addr, 8);
	if((IP_Addr[6] == 0xaa)&&(IP_Addr[7] == 0x55)) {
	...
	for(i=0;i<6;i++) {
		Phy_Addr[i]=*(uint8_t *)(0x1ffff7e8+2*i);//加载物理地址
	}
...
//	UDP_DPORT[1] = 0x70;
	...
	S0_Mode=TCP_SERVER;//加载端口0的工作模式,UDP模式
}
